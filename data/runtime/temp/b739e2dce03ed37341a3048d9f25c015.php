<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:49:"themes/admin_simpleboot3/admin\user\user_crf.html";i:1542099518;s:72:"G:\PHPTutorial\WWW\hx\public\themes\admin_simpleboot3\public\header.html";i:1540550792;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap js-check-wrap">
		...
		<form action="<?php echo url('user/crfpost'); ?>" method="post" class="form-horizontal js-ajax-form margin-top-20">
			...
			<?php if(is_array($result) || $result instanceof \think\Collection || $result instanceof \think\Paginator): if( count($result)==0 ) : echo "" ;else: foreach($result as $k=>$v): ?>
				...
				<?php
					if($v['event_type']=='input')
					{
						...
						?>
						<div class="div_input"><?php echo $v['event_desc']; ?>&nbsp;&nbsp;&nbsp;<input class='input-normal' name="crf[<?php echo $v['id']; ?>]" type="text" value="" ></div>
				<?php
						}elseif($v['event_type_t']=='date'){
						?>
						<div class="div_input"><?php echo $v['event_desc']; ?>&nbsp;&nbsp;&nbsp;<input class="input-normal js-date"   type="text" name="crf[<?php echo $v['id']; ?>]" value="<?php echo date('Y-m-d',time()); ?>" ></div>
				<?php
						...
						<div class="div_select">
							...
							<div class="select_div">
								<?php if(is_array($select['option']) || $select['option'] instanceof \think\Collection || $select['option'] instanceof \think\Paginator): if( count($select['option'])==0 ) : echo "" ;else: foreach($select['option'] as $select_k=>$sel): ?>
									<div class="option_div"><span><?php echo $letter[$select_k]; ?></span>:&nbsp;&nbsp;&nbsp;<input type="<?php echo $v['event_type_t']; ?>" name="crf[<?php echo $v['id']; ?>]">&nbsp;<span class="select_input"><?php echo $sel; ?></span></div>
				...
				?>
						...
						<div class="div_select">
							...
							<table class="table_border" id="crf-<?php echo $v['id']; ?>">
								...
								<tr id="crf-<?php echo $v['id']; ?>-tr">
									<?php if(is_array($table['type']) || $table['type'] instanceof \think\Collection || $table['type'] instanceof \think\Paginator): if( count($table['type'])==0 ) : echo "" ;else: foreach($table['type'] as $table_k=>$tab_t): if($tab_t=='text')
												{
												?>
												<td class="td_border"><input type="text" class="input-normal" name="crf[<?php echo $v['id']; ?>][]" value=""></td>
										<?php
												}elseif($tab_t=='date'){
												?>
												<td class="td_border"><input class="input-normal  js-date" type="text"  name="crf[<?php echo $v['id']; ?>][]" value="<?php echo date('Y-m-d',time()); ?>"></td>
										<?php
												}elseif($tab_t=='radio_or'){
												?>
												<td class="td_border"><input type="radio" name="crf[<?php echo $v['id']; ?>]" value="">是&nbsp;&nbsp;<input type="radio" name="crf[<?php echo $v['id']; ?>][]" value="">否</td>
										<?php
												}else{
												?>
												<td class="td_border"><input type="text" class="input-normal" name="crf[<?php echo $v['id']; ?>][]" value=""></td>
							...
							<a class="btn btn-primary" href="javascript:;" onclick='crfAddTr("crf-<?php echo $v['id']; ?>")'>追加</a>
